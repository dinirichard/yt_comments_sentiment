...
export type InferLocal<GS extends SharedStore> = GS extends { local: infer L }
    ? L extends SharedStore
        ? L
        : {}
    : {};
export type Memory<
    GS extends SharedStore = SharedStore,
    LS extends SharedStore = InferLocal<GS>,
> = GS &
    LS & {
        local: LS;
        clone<T extends SharedStore = SharedStore>(
            forkingData?: T
        ): Memory<GS, LS & T>;
...
function _delete_from_stores(
    ...
    if (further && key in further)
        removed = Reflect.deleteProperty(further, key) || removed;
    return removed;
}

function createProxyHandler<T extends SharedStore>(
    closer: T,
    further?: SharedStore
): ProxyHandler<SharedStore> {
    return {
        get: (target, prop) => {
        ...
        set: (target, prop, value) => {
            if (target._isMemoryObject && prop in target) {
                throw new Error(
                    `Reserved property '${String(prop)}' cannot be set to ${target}`
                );
            }
        ...
        has: (target, prop) =>
            ...
            Reflect.has(closer, prop) ||
            (further ? Reflect.has(further, prop) : false),
        ownKeys: (target) =>
            Array.from(
                new Set([
                    ...Reflect.ownKeys(target),
                    ...Reflect.ownKeys(closer),
                    ...(further ? Reflect.ownKeys(further) : []),
                ])
            ),
        getOwnPropertyDescriptor: (target, prop) => {
            if (Reflect.has(target, prop))
                return Reflect.getOwnPropertyDescriptor(target, prop);
            if (Reflect.has(closer, prop))
                return Reflect.getOwnPropertyDescriptor(closer, prop);
            if (further && Reflect.has(further, prop))
                return Reflect.getOwnPropertyDescriptor(further, prop);
            return undefined;
        },
    };
}
...
export function createMemory<
    GS extends SharedStore,
    LS extends SharedStore = InferLocal<GS>,
>(
    global: GS,
    local: LS = (typeof global.local === "object" && global.local !== null
        ? { ...global.local }
        : {}) as LS
): Memory<GS, LS> {
    ...
    return new Proxy(
        {
            ...
            clone: <T extends SharedStore = SharedStore>(
                ...
                createMemory<GS, LS & T>(global, {
                    ...
                    ...structuredClone(
                        { ...forkingData } /* de-proxyfy it first */
                    ),
                }),
        },
...
> {
    ...
    clone<T extends this>(
        this: T,
        seen: Map<BaseNode, BaseNode> = new Map()
    ): T {
        ...
        for (const [key, value] of this.successors.entries()) {
            cloned.successors.set(
                ...
                Symbol.iterator in value
                    ? value.map((node) =>
                          node && typeof node.clone === "function"
                              ? node.clone(seen)
                              : node
                      )
                    : value
    ...
    async prep(
        memory: Memory<GS, InferLocal<GS>>
    ): Promise<PrepResult | void> {}
    ...
    trigger(
        action: AllowedActions[number],
        forkingData: SharedStore = {}
    ): void {
        if (this.locked) {
    ...
    private listTriggers(
        memory: Memory<GS, InferLocal<GS>>
    ): [AllowedActions[number], Memory<GS, InferLocal<GS> & SharedStore>][] {
        if (!this.triggers.length) {
            return [
                [
                    DEFAULT_ACTION,
                    memory.clone() as Memory<GS, InferLocal<GS> & SharedStore>,
                ],
            ];
        }
        return this.triggers.map((t) => [
            t.action,
            memory.clone(t.forkingData),
        ]);
    }
    ...
    async run(
        ...
        | ReturnType<typeof this.execRunner>
        | ReturnType<typeof this.listTriggers>
    > {
...
> extends BaseNode<GS, PrepResult, ExecResult, AllowedActions> {
    ...
    protected async execRunner(
        ...
        throw new Error(
            "Max retries reached in execRunner without returning or throwing via exec/execFallback."
        );
    }
}
...
export class Flow<
    GS extends SharedStore = SharedStore,
    AllowedActions extends Action[] = Action[],
> extends BaseNode<GS, void, ExecutionTree, AllowedActions> {
    ...
    protected async runTasks<TaskResult>(
        tasks: (() => TaskResult | Promise<TaskResult>)[]
    ): Promise<TaskResult[]> {
    ...
    private async runNodes<CurrentLS extends SharedStore>(
        ...
        return await this.runTasks(
            nodes.map((node) => () => this.runNode(node, memory))
        );
    }

    private async runNode<CurrentLS extends SharedStore>(
        node: BaseNode<GS>,
        memory: Memory<GS, CurrentLS>
    ): Promise<ExecutionTree> {
        ...
        if (currentVisitCount > this.options.maxVisits!) {
            throw new Error(
                `Maximum cycle count (${this.options.maxVisits}) reached for ${node.constructor.name}#${nodeOrder}`
            );
        }
        ...
        for (const [action, nodeMemory] of triggers) {
            const nextNodes = clonedNode.getNextNodes(action);
        ...
        return {
            ...
            triggered: Object.keys(triggered).length > 0 ? triggered : null,
        };
    }
}
...
export class ParallelFlow<
    GS extends SharedStore = SharedStore,
    AllowedActions extends Action[] = Action[],
> extends Flow<GS, AllowedActions> {
    protected async runTasks<TaskResult>(
