...
export class Memory<G extends SharedStore, L extends SharedStore> {
    ...
    >(global: G, local: L = {} as L): Memory<G, L> {
        return new Proxy(new Memory(global, local), {
            ...
            set: (target, prop, value) => {
                if (
                    ["global", "local", "__global", "__local"].includes(
                        prop as string
                    )
                )
                    throw new Error(
                        `Reserved property '${String(prop)}' cannot be set`
                    );
...
> {
    ...
    getNextNodes(action: Action = DEFAULT_ACTION): BaseNode[] {
        ...
        if (
            ...
            console.warn(
                `Flow ends: '${action}' not found in`,
                this.successors.keys()
            );
        }
        return next;
    }
...
> {
    ...
    private async runNode(
        ...
        if (!Array.isArray(triggers)) {
            throw new Error(
                "Node.run with propagate:true must return an array of triggers"
            );
        }
        ...
        const tasks = triggers.map(([action, nodeMemory]) => async () => {
            ...
            return [
                ...
                !nextNodes.length
                    ...
                    : await this.runNodes(
                          nextNodes,
                          nodeMemory as Memory<GlobalStore, SharedStore>
                      ),
            ];
        });
